...
export async function getCandles(productId, numDays = 210) {
  ...
  const data = await cbRequest('GET', `/api/v3/brokerage/products/${productId}/candles?start=${start}&end=${now}&granularity=ONE_DAY`);

  if (!data.candles || data.candles.length === 0) {
    throw new Error(`No candles returned for ${productId}`);
  }
