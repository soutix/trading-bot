...
const DEFAULT_STATE = {
  cash_usd: 500.0, start_cash: 500.0, positions: {},
  cumulative_fees: 0.0, cumulative_slippage: 0.0,
  dry_run: true, last_signals: {}, last_rebalance_timestamp: null,
  initialized_at: null, updated_at: null,
};
...
function computeRequiredTrades(state, targets, prices, config) {
...
function applyPaperTrade(state, trade, config) {
  ...
  if (side === 'BUY') {
    ...
    if (!newState.positions[asset]) newState.positions[asset] = { units: 0, avg_price: slipPrice, weight: 0 };
...
module.exports = { initState, computeEquity, computeRequiredTrades, applyPaperTrade };
