...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get current prices from last_signals (avoid extra Coinbase calls)
    const prices = {};
    const signals = state.last_signals_detail || [];
    signals.forEach(s => { prices[s.asset] = s.price; });

    const equity   = computeEquity(state, prices);
    const invested = equity - state.cash_usd;
    const pnl      = equity - state.start_cash;

    return res.status(200).json({
      initialized         : true,
      total_equity        : parseFloat(equity.toFixed(2)),
      cash_usd            : parseFloat(state.cash_usd.toFixed(2)),
      invested            : parseFloat(invested.toFixed(2)),
      start_cash          : state.start_cash,
      pnl                 : parseFloat(pnl.toFixed(2)),
      pnl_pct             : parseFloat((pnl / state.start_cash * 100).toFixed(2)),
